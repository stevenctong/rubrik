...
<#
.SYNOPSIS
This script will get a list of VMs and do a restore via Live Mount w/migration.
...
.EXAMPLE
./Invoke-LiveMountVMs.ps1
Runs the script and outputs VM info to a CSV file

./Invoke-LiveMountVMs.ps1 -restore $true
...
# Recover VM via Live Mount
Function LiveMount-VM {
  param (
    [CmdletBinding()]
    # Pass in VM Detail object
    [Parameter(Mandatory=$false)]
  ...
  $query = 'mutation VSphereVmInitiateLiveMountV2Mutation($input: VsphereVmInitiateLiveMountV2Input!) {
